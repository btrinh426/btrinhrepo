...
import { addJob, editJob, getJobById } from "../../services/JobService";
...
class Jobs extends React.Component {
  ...
  componentDidMount() {
    ...
    if (!id) {
      ...
      this.setState(() => {
        ...
        formData.skills = formData.skills.map(({ name }) => name);
        formData.skills = formData.skills.join(", ");
        return { formData };
      });
    } else {
      getJobById(id).then(this.onGetJobSuccess).catch(this.onGetJobError);
    }
  }
  ...
  onSubmit = (e) => {
    ...
    } else if (this.state.formData.id) {
      editJob(this.state.formData.id, entries)
        .then(this.onEditJobSuccess)
        .catch(this.onEditJobError);
    } else {
          ...
          <div className="container">
            <form>
              ...
              >
                ...
                >
                  {this.state.jobId || this.state.formData.id
                    ? "Update"
                    : "Submit"}
                </button>
                <Link to="/Home">
