...
class Events extends React.PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    } else if (currentQry && prevQuery === currentQry) {
  ...
  viewMap = () => {
    let eventsLat = this.state.events.map(
      (event) => event.metaData.location.latitude
    );
    let eventsLng = this.state.events.map(
            ...
            <div className="row">
              <div className="col-9">
                <div className=" d-flex justify-content-end pr-5">
                  {" "}
                  <Button color="secondary" onClick={this.viewMap}>
                    View On Map
                  </Button>
                </div>
