...
class Events extends React.Component {
  ...
  onUpdateSuccess = (event) => {
    //this is where i made this update the DOM without an axios call
    let id = event.id;
    let updatedEvent = [];
    ...
    this.setState((prevState) => {
      ...
      let idx = prevState.events.findIndex((event) => event.key == id);
      // events[idx].props.event = event;

      console.log(idx);
      events.splice(idx, 1, updatedEvent.map(this.mapEvent)[0]);
      console.log(events);
