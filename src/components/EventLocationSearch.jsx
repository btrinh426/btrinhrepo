...
const validationSchema = Yup.object().shape({
  ...
  latitude: Yup.string().when("searchOption", {
    is: "latLng",
    then: Yup.string().required("Latitude is required (DD.DDDD or DD MM SS or DD MM.MMM)."),
  }),
  longitude: Yup.string().when("searchOption", {
    is: "latLng",
    then: Yup.string().required("Longitude is required (DD.DDDD or DDD MM SS or DDD MM.MMM)."),
  }),
  ...
  formatSearchLoc = async (values) => {
    ...
    if (values.searchOption === "address") {
      ...
      if (lat.split(" ").length === 3) {
        ...
        console.log("Lat/Lng is not in a recognized format.");
        return { lat: 32.73626630980294, lng: -117.14898687546605 };
      }
    }
  };
