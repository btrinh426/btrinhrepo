  ...
  componentDidMount() {
    if (this.props.location.state) {
      ...
      if (locState.type === "friend_Obj") {
        ...
        newFriend.imageUrl = newFriend.image.imageUrl;
        const skills = newFriend.skills.map(function (item) {
          return item["skillsName"];
        });
        const newSkills = skills.toString();
        newFriend.skills = newSkills;
  ...
  handleClick = (values) => {
    ...
    if (this.props.location.state) {
      updateFriend(payload)
        .then(this.onUpdateFriendSuccess)
        .carch(this.onUpdateFriendError);
    } else {
      postFriend(payload)
