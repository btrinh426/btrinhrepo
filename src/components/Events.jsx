...
class Events extends Component {
  ...
  componentDidMount = () => {
    ...
    // _logger("Events component mounted....getting events database....");
  ...
  clickAddEventButton = (e) => {
    ...
    if (!this.state.eventAddModalShown) {
      ...
      _logger("Event Add/Update Modal already open.");
    }
  };
  ...
  clickSlugLink = (e) => {
    ...
    _logger(this.state.mainEvent);
    _logger(`Clicked slug for event, ID#: ${this.state.mainEvent.id}`);
    if (!this.state.eventAddModalShown) {
  ...
  updateStateWithEvents = (response) => {
    ...
    // _logger("Pagination page size:", this.state.pagination.pageSize);
    ...
    this.setState(() => {
      ...
      // _logger("Update state with new events: ", newState);
      return newState;
    });
  };
  ...
  clickEventCardViewButton = (event) => {
    // _logger(`View event, ID#: ${event.id}`);
  ...
  onAddEventSuccess = (response) => {
    ...
    _logger("Event added, ID# ", response.data.item);
    this.setState(() => {
  ...
  clickSearchEventsButton = (e) => {
    ...
    // _logger("Search Button clicked. Search string: ", this.state.searchEventStartInput);
    ...
    _logger("Search dates: ", searchDates);
  ...
  onGetEventByDateRangeSuccess = (response) => {
    ...
    if (this.state.searchResultDate === "" || this.state.searchEventStartInput !== this.state.searchResultDate) {
      ...
      _logger("Success searching for event (first search result).");
      ...
      _logger("Success search events (subsequent search results");
      const totalCount = response.data.item.totalCount;
  ...
  googleMap = () => {
    // _logger("Updating mainEvent map....");
    let lat, lng;
    // debugger;
