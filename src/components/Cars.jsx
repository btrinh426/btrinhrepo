...
class Cars extends React.Component {
  ...
  onFilterByYear = (e) => {
    ...
    let newCars = carsCopy.filter(
      (car) => car.year === parseInt(e.currentTarget.value)
    );
    ...
    if (parseInt(e.currentTarget.value) > 0) {
      ...
      this.setState((prevState) => {
        return { ...prevState, mappedCars: newCars.map(this.mapCars) };
      });
    } else {
      this.createCarCards();
    }
  ...
  render() {
    return (
      <div>
        ...
        >
          <option defaultValue={"Filter by Year"}>Filter by Year</option>
