...
class Cars extends React.Component {
  ...
  mapCarIfMatching = (aCar) => {
    // console.log("aCar: ", aCar);
    // console.log("this.state.filterOption", this.state.filterOption);
    // console.log(
    //   "this.state.filterOption parseInt",
    //   parseInt(this.state.filterOption)
    // );
    // console.log("aCar.year", aCar.year);
    if (aCar.year === parseInt(this.state.filterOption)) {
      return (
        <SingleCar car={aCar} key={`${aCar.make}${aCar.year}${aCar.model}`} />
      );
    } else {
      console.log("this car didn't match!", aCar);
    }
  };
  ...
  thisYearCars = () => {
    this.setState(() => {
      return {
        mappedCars: this.state.cars.map(this.mapCarIfMatching),
      };
    });
  };
