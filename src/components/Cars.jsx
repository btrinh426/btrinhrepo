...
class Cars extends Component {
  ...
  filterCars = (e) => {
    this.state((prevState) => {
      const yearOfCar = prevState.mappedCars.findIndex(
        (car) => car.year === e.currentTarget.value
      );

      const updatedCars = [...prevState.mappedCars];
      if (yearOfCar >= 0) {
        updatedCars.splice(yearOfCar);
      }
      return { mappedCars: updatedCars };
    });
  };
    ...
    return (
      <React.Fragment>
        ...
        <select className="col-sm-10 custom-select" id="dropDown">
          ...
          <option value="1" onChange={this.filterCars}>
            2019
          </option>
