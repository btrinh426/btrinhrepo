...
Version 5
Add functions to Friends and passing them to FriendFormik as props
When setting state after AJAX CALL, send handlers as props.
-Successfully wired onCancelFormik in Friends and passing its props to FriendFormik
-Now wire onSaveFormik to Friends === onCancelFormik
  ** keep handler within FriendFormik
- Success passed data back to Friends for setState.
- Error calling props at the end of creating a friend
    - cannot create  proptery 'id' on string
    - LINE 145
...
class FriendFormik extends React.Component {
  ...
  propsToFormData(props) {
    console.log("propsToFormData is firing with props: ", props);

    const friend = props.formData;

    //populates each field by first checking to see if there is a default in friend.propertyName
    const item = {
      id: friend.id || "",
      title: friend.title || "",
      bio: friend.bio || "",
  ...
  onSave = (formValues) => {
    ...
    const friend = {
      ...
      statusId: parseInt(formValues.statusId),
      primaryImage: formValues.primaryImage,
      createdBy: "User 1",
      modifiedBy: "Other User 1",
      skills: ["React", "Jack Hammer", "High Jumper"],
      //   others: {
    ...
    if (this.state.formData.id) {
      ...
      add(friend)
        .then((friend) => {
          console.log("addAJAX.then is firing with Data: ", friend);

          let tempFriend = friend.config.data;
          tempFriend.id = friend.data.item;

          console.log("tempFriend is: ", tempFriend);

          this.showSaveSuccess(tempFriend);
          // Modify state to reflect assigned id value

          that.props.onSave({ ...friend, id: friend.item });
        })
        .catch(this.onSaveErrorGeneric);
    }
  };

  // callAddAsyncFromFriends = (friendReturned) => {
  //   this.props.callAddAsyncFromFriends(friendReturned);
  // };

  onCancel = (event) => {
    console.log("onCancel is firing from FriendFormik", event);
