...
class Friends extends React.Component {
  ...
  onDeleteClicked = (e) => {
    ...
    const pointerToFx = this.onDeleteSuccessCurry(id);
    ...
    return (data) => {
      ...
      this.setState((prevState) => {
        const indexOfFriends = prevState.mappedFriends.findIndex(
          (oneFriend) => oneFriend.id === data - id.id
        );
      ...
      console.log("Successful Delete", data - id);
    };
  };
  ...
  onSearchClicked = (e) => {
    ...
    this.props.history.push(
      "/friends/search?pageIndex=" + index + "&pageSize=" + size + "&q=" + input
    );
    friendService
      .searchForFriend(0, 4, input)
      .then(this.onSearchSuccess)
      .catch(this.onSeachFail);
  };

  onSearchSuccess = (searchRes) => {
    console.log("searched term:", searchRes.data.item.pagedItems);
    this.setState(() => {
      return {
