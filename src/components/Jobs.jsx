...
class Jobs extends Component {
  ...
  clickAddJobButton = (e) => {
    e.preventDefault();
    // _logger("Click Add Job button....");
    this.props.history.push("/jobs/new");
  };

  clickShowHideJobs = (e) => {
    // _logger("Clicked Show/Hide Jobs button.");
  ...
  onGetJobDatabaseSuccess = (response) => {
    // _logger("Success getting jobs from database.");
  ...
  updateStateWithJobs = (response) => {
    ...
    this.setState(
      ...
      () => {
        // _logger("Finished setting job array in state.");
      }
    );
  };
  ...
  onSearchJobsSuccess = (response) => {
    ...
    if (!this.state.isSearchResult || this.state.searchString !== document.getElementById("searchJobsInput").value) {
      ...
      _logger("Success searching for job.");
