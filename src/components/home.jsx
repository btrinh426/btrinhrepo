...
class Home extends React.Component {
    ...
    componentDidMount = () => {
        UserService.usersCurrent().then(this.mountedData).catch(this.unsuccessfulMount)
    }

    mountedData = (currentUser) => {
        UserService.usersId(currentUser.data.item.id).then(this.onCurrentUser).catch(this.unsuccessfulMount)
    }
    ...
    onLogOutButton = () => {
        UserService.usersLogout().then(this.onLogOutSuccess).catch(this.onLogOutError)
    }
