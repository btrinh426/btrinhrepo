...
class CarsChkBox extends React.Component {
    state = {
        cars: [
            {
    ...
    mapCarYear = (oneCarYear)=> {
        return (
            
            <div key={`cars-${i}`} className="card col-md-3">
                <input type="checkbox" name={this.state.cars.year} />
            </div>
    ...
    render() {
        ...
        return(
            <div className="row">
            {this.state.cars.map(this.mapCarYear)} 
            </div>
        )
    }
}

export default CarsChkBox